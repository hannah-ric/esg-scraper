    ...
    async def __call__(self, request: Request, call_next):
        ...
        if path_parts and path_parts[0] in API_VERSIONS:
            ...
            if api_version.is_deprecated:
                ...
                response.headers["Link"] = f'</{DEFAULT_VERSION}/>; rel="successor-version"'
                
                if api_version.days_until_sunset and api_version.days_until_sunset <= SUNSET_WARNING_DAYS:
                    response.headers["Warning"] = f'299 - "This API version will sunset in {api_version.days_until_sunset} days"'
...
def setup_versioning(app: FastAPI):
    ...
    async def get_versions():
        ...
        return {
            ...
            "supported": [v for v, info in API_VERSIONS.items() 
                         if not APIVersion(v).is_sunset]
        }
