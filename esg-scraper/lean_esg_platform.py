...
sentiment_analyzer = pipeline(
    ...
    device=-1  # CPU for cost efficiency
)
...
class KeywordScorer:
    ...
    def __init__(self):
        self.keywords = {
            "environmental": {
                "high_weight": ["carbon neutral", "net zero", "renewable energy", "science-based targets"],
                "medium_weight": ["emissions", "climate", "sustainability", "recycling"],
                "low_weight": ["environment", "green", "eco", "conservation"]
            },
            "social": {
                "high_weight": ["human rights", "diversity equity inclusion", "employee wellbeing"],
                "medium_weight": ["diversity", "safety", "community", "training"],
                "low_weight": ["social", "employee", "workplace", "engagement"]
            },
            "governance": {
                "high_weight": ["board independence", "executive compensation", "audit committee"],
...
class EnhancedESGEngine(LeanESGEngine):
    ...
    async def analyze(self, content: str, company_name: str = None, 
                     ...
                     industry_sector: str = None, extract_metrics: bool = True) -> Dict[str, Any]:
    ...
    def _extract_all_metrics(self, content: str, requirements: Dict[Framework, List[DisclosureRequirement]]) -> List[Dict[str, Any]]:
    ...
    def _determine_severity(self, requirement: DisclosureRequirement, framework: Framework, industry: str = None) -> str:
        ...
        if industry:
            ...
            if industry_lower in ['energy', 'utilities', 'oil & gas'] and 'emission' in requirement.description.lower():
                ...
                return 'high'
            elif industry_lower in ['manufacturing', 'automotive'] and 'supply chain' in requirement.description.lower():
                return 'high'
...
class EnhancedKeywordScorer(KeywordScorer):
    ...
    def __init__(self):
        ...
        self.keywords['environmental'].update({
            'csrd_aligned': ["double materiality", "transition plan", "climate scenario", 
                           "taxonomy alignment", "dnsh", "do no significant harm"],
            'tcfd_aligned': ["physical risk", "transition risk", "climate opportunity",
                           "scenario analysis", "2 degree scenario", "1.5 degree"],
            'gri_aligned': ["material topics", "stakeholder impact", "environmental management"],
            'sasb_aligned': ["financially material", "industry-specific", "operational efficiency"]
        })
        
        self.keywords['social'].update({
            'csrd_aligned': ["due diligence", "value chain workers", "affected communities",
                           "consumer safety", "data protection", "gdpr"],
            'gri_aligned': ["stakeholder engagement", "material topics", "human rights assessment",
                          "collective bargaining", "local communities"],
            'sasb_aligned': ["human capital", "social capital", "customer welfare"]
        })
        
        self.keywords['governance'].update({
            'csrd_aligned': ["sustainability governance", "remuneration policy", "business conduct",
                           "whistleblower protection", "anti-corruption"],
            'sasb_aligned': ["systemic risk", "competitive behavior", "regulatory capture",
                           "political influence", "lobbying"],
            'tcfd_aligned': ["climate governance", "board oversight", "risk committee"]
        })
...
async def register(registration: UserRegistration):
    ...
    # Set subscription
    redis_client.hset(f"subscription:{user_id}", mapping={
        "tier": "free",
...
async def get_analysis_gaps(
    ...
    for gap in gaps:
        ...
        if framework not in grouped_gaps:
            grouped_gaps[framework] = {"critical": [], "high": [], "medium": [], "low": []}
...
class EnhancedDatabaseManager(DatabaseManager):
    ...
    async def save_analysis(self, user_id: str, source: str, result: Dict[str, Any], 
                          industry_sector: str = None, reporting_period: str = None):
