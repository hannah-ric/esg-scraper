...
class MetricStandardizer:
    ...
    def standardize_metrics(self, metrics: List[Dict[str, Any]], 
    ...
    def _standardize_single_metric(self, metric: Dict[str, Any], 
    ...
    def _categorize_metric(self, name: str, unit: str) -> MetricCategory:
        ...
        # Energy
        if any(term in name_lower for term in ["energy", "electricity", "power"]) or \
           any(term in unit_lower for term in ["kwh", "mwh", "gwh", "joule"]):
            return MetricCategory.ENERGY
        
        # Water
        if any(term in name_lower for term in ["water", "consumption"]) and \
           any(term in unit_lower for term in ["liter", "litre", "m3", "cubic"]):
        ...
        # Social
        if any(term in name_lower for term in ["employee", "worker", "diversity", 
                                               "training", "safety", "incident"]):
            return MetricCategory.SOCIAL
        
        # Governance
        if any(term in name_lower for term in ["board", "governance", "ethics", 
                                               "compliance", "audit"]):
    ...
    def _convert_to_base_unit(self, value: float, from_unit: str, to_unit: str) -> float:
    ...
    def _extract_period(self, metric: Dict[str, Any], context: Dict[str, Any] = None) -> str:
    ...
    def _is_target_metric(self, name: str) -> bool:
        ...
        target_keywords = ["target", "goal", "commitment", "by 2030", "by 2040", 
                          "by 2050", "will", "plan to", "aim to"]
        name_lower = name.lower()
    ...
    def _build_metric_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            "emissions_reduction": re.compile(
                r"reduced?.*emissions?.*by\s+(\d+\.?\d*)\s*(%|percent)",
                re.IGNORECASE
            ),
            "renewable_energy": re.compile(
                r"(\d+\.?\d*)\s*(%|percent).*renewable\s+energy",
                re.IGNORECASE
            ),
            # Add more patterns
        }
    ...
    def _build_validation_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "percentage": {
                "min": 0,
                "max": 100,
                "units": [MetricUnit.PERCENTAGE]
            },
            # Add more rules
        }
    
    def aggregate_metrics(self, metrics: List[Dict[str, Any]], 
                         aggregation_type: str = "sum") -> Dict[str, Any]:
        """
