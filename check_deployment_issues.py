...
def check_imports():
    ...
    code, stdout, _ = run_command(
        "grep -r 'from \\.' --include='*.py' --exclude-dir=venv ."
    )
    if stdout:
...
def check_env_vars():
    ...
    for pattern in secret_patterns:
        code, stdout, _ = run_command(
            f"grep -r '{pattern}' --include='*.py' --exclude-dir=venv ."
        )
...
def check_dockerfile():
    ...
    if (
        ...
        and content.rindex("USER root") > content.rindex("USER appuser")
    ...
    if "COPY . ." in content and "requirements.txt" not in content:
        issues.append(
            "Dockerfile copies all files before installing requirements - inefficient caching"
        )
...
def check_memory_usage():
    ...
    code, stdout, _ = run_command(
        "grep -r 'transformers' --include='*.py' --exclude-dir=venv . | grep -v '#'"
    )
    if stdout and "disable_transformers" not in stdout:
        issues.append(
            "Transformers library is imported but might cause memory issues on small instances"
        )

    # Check for large in-memory operations
    code, stdout, _ = run_command(
        "grep -r 'read()' --include='*.py' --exclude-dir=venv . | grep -v 'readline'"
    )
    if stdout:
        issues.append(
            "Found .read() operations that might load large files into memory"
        )
...
def check_api_consistency():
    ...
    code, stdout, _ = run_command(
        "grep -r 'HTTPException' --include='*.py' --exclude-dir=venv ."
    )
    if not stdout:
        print(
            "⚠️  No HTTPException usage found - might have inconsistent error handling"
        )

    # Check for proper async usage
    code, stdout, _ = run_command(
        "grep -r 'def ' --include='*.py' --exclude-dir=venv . | grep -v 'async def' | grep '@app'"
    )
    if stdout:
