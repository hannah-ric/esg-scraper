...
class RedisMigrator:
    ...
    def connect_source(self):
        ...
        try:
            self.source_client = redis.from_url(self.source_url, decode_responses=True)
    ...
    def connect_target(self):
        ...
        try:
            if self.target_url.startswith("rediss://"):
                ...
                self.target_client = redis.from_url(
                    self.target_url, decode_responses=True
                )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if self.migration_stats["keys_failed"] == 0:
            if (
                self.migration_stats["keys_migrated"]
                == self.migration_stats["keys_scanned"]
            ):
