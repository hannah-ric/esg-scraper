...
class MongoDBBackupVerifier:
    ...
    async def verify_backup_exists(self):
        ...
        try:
            ...
            if not do_token:
                logger.warning(
                    "⚠️  DIGITALOCEAN_ACCESS_TOKEN not set, cannot verify backup configuration"
                )
                return False
                ...
                if response.status_code == 200:
                    ...
                    for db in databases:
                        if cluster_name in db.get("connection", {}).get("uri", ""):
                            ...
                            logger.info(
                                f"   - Hour: {backup_window.get('hour', 'N/A')}"
                            )
                            logger.info(
                                f"   - Retention: 7 days (DigitalOcean default)"
                            )
                            return True
                    ...
                    logger.error(
                        f"❌ Failed to query DigitalOcean API: {response.status_code}"
                    )
    ...
    async def generate_backup_report(self):
        ...
        critical_passed = (
            report["checks"]["connection"] and report["checks"]["data_integrity"]
        )
